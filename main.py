...
def get_remaining(prayer_time):
    ...
    try:
        ...
        if target <= now:
            ...
            return f"انقضت منذ: {hours}س {minutes}د"
...
def format_times(city, day_num=None):
    ...
    def format_remaining(time):
        rem = get_remaining(time)
        if not rem:
            return ""
        if rem.startswith("انقضت"):
            return rem
        return f"متبقي: {rem}"
...
🌙 الفجر    {fajr}  {format_remaining(fajr)}
🕐 الظهر    {zuhr}  {format_remaining(zuhr)}
🌅 العصر    {asr}  {format_remaining(asr)}
🌇 المغرب   {maghrib}  {format_remaining(maghrib)}
🌃 العشاء   {isha}  {format_remaining(isha)}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
...
def format_prayer(city, prayer_name):
    ...
    if remaining.startswith("انقضت"):
        return f"🕌 صلاة {actual_name}\n\n⏰ الوقت: {time} بتوقيت {city}\n{remaining}"
    return f"🕌 صلاة {actual_name}\n\n⏰ الوقت: {time} بتوقيت {city}\nمتبقي: {remaining}"
...
def format_next_prayer(city):
    ...
    for name, time in prayers:
        if time == "-":
            continue
        remaining = get_remaining(time)
        if remaining and not remaining.startswith("انقضت"):
...
async def send_reminders(context: ContextTypes.DEFAULT_TYPE):
    ...
    for user_id, city in users:
        ...
        prayers = {
            "الفجر": city_day.get("الفجر", ""),
            "الظهر": city_day.get("الظهر", ""),
